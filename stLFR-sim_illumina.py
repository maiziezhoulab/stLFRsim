...
This is added trace version, search “add trace” to find out which parts were changed
...
def SIMSR(start, end, Par, lib, jobid, reftitle): # add trace
    ...
    for i in range(len(MolSet_cand)):
        ...
        seqname = reftitle[MolSet_cand[i].seqidx] # add trace
        Seq_rand_qual = []
        Barcode_rand_qual = []
        ...
        for j in range(len(Totalreads)):
            ...
            f_reads2.write((read2seq[:Par.SR] + "\n").encode("utf-8"))
            f_reads2.write(("+\n").encode("utf-8"))
            f_reads2.write((read2qual[:Par.SR] + "\n").encode("utf-8"))
    f_reads1.close()
    f_reads2.close()
    return None
...
def pairend(
    ...
    return Short_reads_PE(
        read1seq, read1qual, start_for, end_for, read2seq, read2qual, start_rev, end_rev
    )
